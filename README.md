...
```
build-v2/
├── index.html              # Main landing page. Hero, four categories, four commitments, about, for clarity, two-path intake, footer.
...
- **v2.7** (2026-04-27) Compliance fix to header subheader. Changed the persistent header line from "A personal practice of Taran Aujla" to "A personal practice of Taran Aujla, Salesperson · HomeLife G1 Realty Inc., Brokerage." This places the brokerage identification with the required "Brokerage" descriptor in the persistent navigation, ensuring brokerage prominence is achieved at every scroll position on every page, in the same visual frame as the Primegate brand mark. Applied to every deployed page (index.html, notes/index.html, notes/_template.html, notes/deals-between-signing-and-closing.html, thanks/index.html). Addresses RECO Bulletin 5.3 prominence requirement for online advertising. No other content, design, or structural changes.
